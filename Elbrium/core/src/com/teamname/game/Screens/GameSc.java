...
public class GameSc implements Screen {
    ...
    private static final int entityX=Main.BACKGROUND_WIDTH/2-entityRad;
    private static final int entityY=Main.BACKGROUND_HEIGHT/2-entityRad;
...
//        multiplayer.draw(batch);
    ...
    public void player_player_collision(){
        for(String s : multiplayer.meta_players){
            if(player.bounds.Overlaps(multiplayer.getBounds()))Gdx.app.error("MP","overlaps");
        }
    }
    ...
    public void multiplayerBounds(){
        for(Message msg : multiplayer.players){
            Circle bounds = new Circle(new Point2D(msg.x+player.R*2,msg.y+player.R*2),player.R);
            if(player.bounds.Overlaps(bounds))Gdx.app.error("MP","overlaps");
        }
    }
